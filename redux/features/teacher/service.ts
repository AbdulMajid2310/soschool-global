...
export const teacherService = {
  ...
  async getOneBySchool(schoolId: string, teacherId: string) {
    const response = await api.get(
      `/school-teachers/school/${schoolId}/${teacherId}`,
    );
    return response.data;
  },
  ...
  async importCsv(schoolId: string, file: File) {
    ...
    const response = await api.post(
      `/school-teachers/import/${schoolId}`,
      formData,
      {
        headers: {
  ...
  // Metode baru untuk hapus massal berdasarkan schoolId dan array userId
  async deleteBulk(schoolId: string, userIds: string[]) {
    const response = await api.delete(
      `/school-teachers/bulk-remove/${schoolId}`,
      {
        data: { userIds }, // Axios DELETE membutuhkan body di dalam properti 'data'
      },
    );
    return response.data;
  },
