...
import { School, CreateSchoolRequest, UpdateSchoolRequest } from "./types";
  ...
  create: async (data: CreateSchoolRequest): Promise<School> => {
    try {
      ...
      Object.entries(data).forEach(([key, value]) => {
        if (value !== undefined && value !== null && key !== 'avatar' && key !== 'background') {
      ...
      if (error.response) {
        ...
        throw Array.isArray(serverMessage) ? serverMessage.join(", ") : serverMessage || "Gagal mendaftarkan sekolah";
...
update: async (schoolId: string, data: UpdateSchoolRequest): Promise<School> => {
  try {
    const formData = new FormData();
    ...
    const serverMessage = error.response?.data?.message;
    throw Array.isArray(serverMessage) ? serverMessage.join(", ") : serverMessage || "Gagal memperbarui sekolah";
  }
},
