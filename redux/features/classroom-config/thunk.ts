...
export const fetchClassroomConfigs = createAsyncThunk(
    ...
    async ({ schoolId, periodId }: { schoolId: string, periodId?: string }, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue(err.response?.data?.message || 'Gagal mengambil data konfigurasi');
        }
    }
);
...
export const fetchClassroomConfigById = createAsyncThunk(
    ...
    async ({ id, schoolId }: { id: string, schoolId: string }, { rejectWithValue }) => {
        try {
            ...
            return rejectWithValue(
                msg || 'Gagal mengambil detail konfigurasi kelas'
            );
        }
    }
);
...
export const registerClassroomConfig = createAsyncThunk(
    ...
    async (payload: CreateClassroomConfigPayload, { rejectWithValue, dispatch }) => {
        try {
            const response = await classroomConfigService.create(payload);
            const resData = response.data as ApiResponse<ClassroomConfig>;
            dispatch(fetchClassroomConfigs({ schoolId: payload.schoolId, periodId: payload.periodId }));
            return resData.data;
        } catch (err: any) {
            const msg = err.response?.data?.message;
            return rejectWithValue(Array.isArray(msg) ? msg[0] : msg || 'Gagal menyimpan konfigurasi');
        }
    }
);
...
export const updateClassroomConfig = createAsyncThunk(
    ...
    async ({ id, ...payload }: { id: string } & CreateClassroomConfigPayload, { rejectWithValue, dispatch }) => {
        try {
            ...
            const msg = err.response?.data?.message;
            return rejectWithValue(Array.isArray(msg) ? msg[0] : msg || 'Gagal memperbarui konfigurasi');
        }
    }
);
...
export const removeClassroomConfig = createAsyncThunk(
    ...
    async ({ id, schoolId, periodId }: { id: string, schoolId: string, periodId?: string }, { rejectWithValue }) => {
        try {
            await classroomConfigService.delete(id, schoolId);
            return id;
        } catch (err: any) {
            return rejectWithValue(err.response?.data?.message || 'Gagal menghapus konfigurasi');
        }
    }
);