...
import {
  FiMenu, FiSearch, FiBell, FiUser, FiSettings, FiLogOut, FiRepeat, FiChevronDown
} from 'react-icons/fi';
  ...
  return (
    <header className="h-16 md:h-20 w-full fixed top-0 z-50 flex items-center justify-between px-4 md:px-8 bg-white/80 dark:bg-gray-900 backdrop-blur-xl border-b border-slate-200 dark:border-blue-900/20 shadow-sm">
      ...
      <div className="flex items-center gap-4 shrink-0">
        <button onClick={onMenuClick} className="lg:hidden p-2 text-slate-600 dark:text-blue-400 hover:bg-slate-100 dark:hover:bg-blue-500/10 rounded-xl">
          <FiMenu size={22} />
        </button>

        <Link href="/home" className="flex items-center gap-3">
          <img src="/images/logo.png" alt="Logo" className="w-8 h-8 md:w-9 md:h-9 object-contain" />
          <div className="hidden sm:block">
            ...
            <p className="text-[7px] font-bold text-slate-400 uppercase tracking-widest italic">Student Hub</p>
          </div>
        </Link>
      ...
      <div className="flex items-center gap-3 relative" ref={dropdownRef}>
        ...
        <div className="hidden xl:flex items-center gap-2 mr-2">
          <QuickLink href="/home" active={isNavActive('/home')} icon={<IoHome />} />
          <QuickLink href="/marketplace" active={isNavActive('/marketplace')} icon={<SiHomeassistantcommunitystore />} />
          <QuickLink href="/bank-soal" active={isNavActive('/bank-soal')} icon={<BsBank2 />} />
        </div>
        ...
        <div className="flex items-center gap-2 border-l border-slate-200 dark:border-blue-900/20 pl-3">
          ...
          <IconButton icon={<FiBell />} active={activeDropdown === 'notif'} onClick={() => toggleDropdown('notif')} />
          ...
          <div className="relative">
            ...
            >
              <img
                src={profile?.user?.avatar || "https://i.pravatar.cc/150?u=majid"}
                className="w-8 h-8 rounded-lg object-cover border border-slate-200 dark:border-blue-900/30"
                alt="User"
              />
              <FiChevronDown className={`text-slate-400 transition-transform ${activeDropdown === 'profile' ? 'rotate-180' : ''}`} size={14} />
            </button>
            ...
            {activeDropdown === 'profile' && (
              <div className="absolute top-12 right-0 w-64 p-2 bg-white dark:bg-[#0a0f1d] border border-slate-200 dark:border-blue-900/30 rounded-2xl shadow-2xl animate-in fade-in slide-in-from-top-2">
                <div className="px-3 py-2 border-b border-slate-100 dark:border-blue-900/10 mb-2">
                  <p className="text-[10px] font-black uppercase text-slate-900 dark:text-white truncate">{profile?.user?.username}</p>
                  <p className="text-[8px] text-slate-400 uppercase tracking-tighter">{profile?.user?.email}</p>
                </div>

                <DropdownAction icon={<FiUser />} title="Profil Saya" onClick={() => router.push('/settings/profile')} />
                <DropdownAction icon={<FiRepeat className="text-blue-500" />} title="Ganti Peran / Sekolah" onClick={() => router.push('/select-role')} />

                <div className="flex items-center justify-between px-3 py-2 bg-slate-50 dark:bg-white/5 rounded-xl my-1">
                  <span className="text-[9px] font-bold uppercase text-slate-500 italic">Dark Mode</span>
                  <ThemeToggle />
                </div>

                <div className="h-px bg-slate-100 dark:bg-blue-900/10 my-1" />
                <DropdownAction icon={<FiLogOut className="text-red-500" />} title="Keluar Sesi" onClick={() => { dispatch(logoutUser()); router.push('/login'); }} />
              </div>
            )}
          </div>
        </div>
...
const QuickLink = ({ href, active, icon }: any) => (
  <Link href={href} className={`p-2.5 rounded-xl transition-all ${active ? 'text-blue-500 bg-blue-500/10' : 'text-slate-400 hover:text-blue-500'}`}>
    <span className="text-xl">{icon}</span>
  </Link>
);

const IconButton = ({ icon, count, active, onClick }: any) => (
  <button onClick={onClick} className={`relative p-2 rounded-xl border transition-all ${active ? 'bg-blue-600 border-blue-400 text-white' : 'bg-slate-50 dark:bg-white/5 border-transparent text-slate-400'}`}>
    <span className="text-lg">{icon}</span>
    {count && <span className="absolute -top-1 -right-1 bg-red-600 text-[8px] text-white w-4 h-4 flex items-center justify-center rounded-full border-2 border-white dark:border-gray-900">{count}</span>}
  </button>
);

const DropdownAction = ({ icon, title, onClick }: any) => (
  <button onClick={onClick} className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-blue-500/10 transition-all group">
    <span className="text-slate-400 group-hover:text-blue-500 transition-colors">{icon}</span>
    <span className="text-[10px] font-black uppercase italic text-slate-700 dark:text-slate-300">{title}</span>
  </button>
);
