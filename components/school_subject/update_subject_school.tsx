...
const UpdateSubjectModal = ({
  ...
  useEffect(() => {
    if (isOpen && subjectData) {
      setFormData({
        ...
        schoolClassroomId:
          subjectData.classroom?.schoolClassroomId ||
          subjectData.schoolClassroomId ||
          "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        typeof err === "string"
          ? err
          : err?.message || "Gagal memperbarui mata pelajaran";
  ...
  const selectedClassroom = classrooms.find(
    (c) => c.schoolClassroomId === formData.schoolClassroomId,
  );
                ...
                {classrooms.map((cls) => (
                  <button
                    ...
                    onClick={() => {
                      setFormData({
                        ...formData,
                        schoolClassroomId: cls.schoolClassroomId,
                      });
                      setIsDropdownOpen(false);
                    }}
                  >
                    <span className="text-sm">{cls.name}</span>
                    {formData.schoolClassroomId === cls.schoolClassroomId && (
