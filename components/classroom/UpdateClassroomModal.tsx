...
import { updateClassroom } from "@/redux/features/classroom/thunk";
import {
  HiOutlineXMark,
  HiOutlineHashtag,
  HiOutlineHomeModern,
  ...
  HiOutlineUsers, // Ikon untuk kapasitas
...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  data: SchoolClassroom | null;
}

export const UpdateClassroomModal = ({ isOpen, onClose, data }: Props) => {
  const dispatch = useAppDispatch();
  const { loading } = useAppSelector((state) => state.classroom);
  const [isLevelOpen, setIsLevelOpen] = useState(false);

  // Update state untuk menampung capacity
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    dispatch(updateClassroom(payload))
      ...
      .catch((err) => {
        toast.error(err || "Gagal memperbarui data");
      });
  };
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center p-6">
      ...
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-4xl shadow-2xl border border-transparent dark:border-slate-800 overflow-hidden animate-in zoom-in-95">
        <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
          <div className="space-y-1">
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">
              ID: {data.schoolClassroomId.split("-")[0]}...
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6">
          ...
          <button
            disabled={loading}
            className="w-full py-6 bg-amber-500 hover:bg-amber-600 text-white rounded-4xl font-black uppercase tracking-widest text-[11px] transition-all flex items-center justify-center gap-3 shadow-xl disabled:opacity-50"
            type="submit"
            title="simpan"
          >
            {loading ? (
              ...
              <>
                <HiOutlineCheckCircle size={20} /> Simpan Perubahan
              </>
            )}
          </button>
        </form>
