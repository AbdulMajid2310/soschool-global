            ...
            >
              <div className="flex flex-col items-center">
                <div className="relative mb-6">
                  ...
                  <div
                    className={`absolute -bottom-1 -right-1 w-8 h-8 rounded-xl border-4 border-white dark:border-gray-900 flex items-center justify-center shadow-lg ${s.isActive ? "bg-emerald-500" : "bg-rose-500"}`}
                  >
                    {s.isActive ? (
                      <FiCheck className="text-white size-3 stroke-[4px]" />
                    ) : (
                      <FiX className="text-white size-3 stroke-[4px]" />
                    )}
                  </div>
                </div>
                ...
                <div className="w-full space-y-3 mb-8">
                  <div className="flex items-center gap-3 text-gray-500 dark:text-gray-400">
                    ...
                    <span className="text-xs font-bold font-mono">
                      {s.nip || s.employeeId || "ID BELUM DISET"}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-gray-500 dark:text-gray-400">
                    <FiPhone className="size-4 shrink-0" />
                    <span className="text-xs font-bold">
                      {s.user.phone || "NO TELEPON —"}
                    </span>
                  </div>
                </div>
                ...
                <div className="w-full grid grid-cols-3 gap-2">
                  ...
                  <button
                    type="button"
                    title="edit"
                    className="flex items-center justify-center p-3 bg-slate-50 dark:bg-gray-800 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all"
                  >
      ...
      {!loading && filteredStaffs.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 bg-white dark:bg-gray-900 rounded-[3rem] border-4 border-dashed border-gray-50 dark:border-gray-800">
          ...
          <p className="text-gray-400 font-bold mt-2">
            Belum ada staff yang terdaftar di sekolah ini
          </p>
        </div>
      )}
    </div>
