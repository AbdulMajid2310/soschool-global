...
import { importStaffCsv } from "@/redux/features/staff/thunks";

export default function ImportStaffCsv() {
  const dispatch = useAppDispatch();
  const [file, setFile] = useState<File | null>(null);
  const schoolId = useSchoolId();
  const { loading, importReport } = useAppSelector(
  ...
  const handleImport = async () => {
    ...
    try {
      await dispatch(importStaffCsv({ file, schoolId })).unwrap();
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-gray-500 font-bold text-sm">
            Upload file .xlsx dengan format kolom yang sesuai
          </p>
        </div>
      </div>
      ...
      <div className="relative group border-4 border-dashed border-gray-100 dark:border-gray-800 rounded-[3rem] p-12 transition-all hover:border-indigo-500/30 flex flex-col items-center">
        ...
        <FiFile
          size={48}
          className="text-gray-300 group-hover:text-indigo-400 transition-colors mb-4"
        />
        ...
        <div className="mt-6 p-6 bg-slate-50 dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div className="bg-white dark:bg-gray-900 p-4 rounded-2xl">
              <p className="text-xl font-black">{importReport.total}</p>
              <p className="text-[10px] font-bold text-gray-400 uppercase">
                Total
              </p>
            </div>
            <div className="bg-white dark:bg-gray-900 p-4 rounded-2xl text-emerald-500">
              <p className="text-xl font-black">{importReport.success}</p>
              <p className="text-[10px] font-bold text-gray-400 uppercase">
                Sukses
              </p>
            </div>
            <div className="bg-white dark:bg-gray-900 p-4 rounded-2xl text-rose-500">
              <p className="text-xl font-black">{importReport.failed}</p>
              <p className="text-[10px] font-bold text-gray-400 uppercase">
                Gagal
              </p>
            </div>
