...
export default function SchoolPeriodManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = typeof err === 'string' ? err : (err?.message || 'Gagal menyimpan data');
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter italic">
              Master <span className="text-indigo-600 dark:text-indigo-500 not-italic">Periode</span>
            </h1>
            <p className="text-slate-400 dark:text-slate-500 text-xs font-medium">
              Kelola tahun ajaran dan semester aktif untuk seluruh sistem pendidikan.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {loading && periods.length === 0 ? (
          ...
          periods.map((p) => (
            <div
              key={p.periodId}
              className={`flex flex-col md:flex-row items-center justify-between p-6 bg-white dark:bg-slate-900 border transition-all duration-300 rounded-3xl ${p.isActive ? 'border-indigo-500 ring-4 ring-indigo-500/5 shadow-lg' : 'border-slate-100 dark:border-slate-800'
                }`}
            >
              <div className="flex items-center gap-5">
                <div className={`p-4 rounded-2xl ${p.isActive ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                  <HiOutlineCalendarDays size={24} />
                </div>
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-white">{p.academicYear}</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Semester {p.semester}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 mt-4 md:mt-0">
                <button
                  ...
                  disabled={loading}
                  className={`px-5 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${p.isActive ? 'bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white' : 'bg-slate-900 dark:bg-indigo-600 text-white hover:opacity-80'
                    }`}
                >
                ...
                <button
                  onClick={() => {
                    setFormData({ periodId: p.periodId, academicYear: p.academicYear, semester: p.semester as any });
                    setIsModalOpen(true);
                  }}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-[2.5rem] p-10 relative shadow-2xl animate-in zoom-in-95 duration-200">
            <h2 className="text-2xl font-black mb-6 dark:text-white italic">Konfigurasi Periode</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Tahun Akademik</label>
                <input
                  required
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Pilih Semester</label>
                <div className="grid grid-cols-2 gap-2 p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl">
                  {['GANJIL', 'GENAP'].map((s) => (
                    <button
                      key={s}
                      type="button"
                      onClick={() => setFormData({ ...formData, semester: s as any })}
                      className={`py-3 rounded-xl font-black text-xs transition-all ${formData.semester === s ? 'bg-white dark:bg-slate-800 text-indigo-600 shadow-sm' : 'text-slate-400'
                        }`}
                    >
                      {s}
                    </button>
