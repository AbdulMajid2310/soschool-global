      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 text-balance">
              Teacher Monitoring
            </span>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2 md:pb-0">
          {days.map((day) => (
            <button
              ...
              className={`px-6 py-3 rounded-2xl text-[10px] font-black transition-all shrink-0 tracking-widest border-2 ${
                activeDay === day
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {filteredData.length > 0 ? (
          filteredData.map((group, idx) => {
            ...
            const isGroupActiveNow =
              group.day === todayName &&
              group.sessions.some(
                (s) =>
                  curT >= s.startTime.substring(0, 5) &&
                  curT <= s.endTime.substring(0, 5),
              );

            return (
              <div
                key={idx}
                className={`relative flex flex-col p-5 rounded-3xl border-2 transition-all duration-500 group ${
                  isGroupActiveNow
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div
                    className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] ${isGroupActiveNow ? "bg-white/20 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}
                  >
                    {group.day}
                  </div>
                </div>
                ...
                <div className="mb-6 flex items-center gap-3">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg transition-transform group-hover:rotate-3 ${isGroupActiveNow ? "bg-white text-blue-600" : "bg-blue-600 text-white shadow-lg"}`}
                  >
                    {group.teacher?.user?.username?.charAt(0)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-sm font-black uppercase leading-tight truncate ${isGroupActiveNow ? "text-white" : "text-slate-900 dark:text-white"}`}
                    >
                      {group.teacher?.user?.username}
                    </p>
                    <p
                      className={`text-[10px] font-bold ${isGroupActiveNow ? "text-white/60" : "text-slate-400"}`}
                    >
                ...
                <div className="flex-1 mb-6">
                  <h3
                    className={`text-xl font-black italic uppercase tracking-tighter leading-none mb-1 ${isGroupActiveNow ? "text-white" : "text-slate-900 dark:text-white"}`}
                  >
                    {group.subject?.name}
                  </h3>
                  <div
                    className={`text-[10px] font-bold uppercase flex items-center gap-1.5 ${isGroupActiveNow ? "text-white/70" : "text-blue-600"}`}
                  >
                    <HiOutlineAcademicCap size={14} />{" "}
                    {group.classroom.classroom.name}
                  </div>
                </div>

                <div
                  className={`space-y-2 p-3 rounded-2xl border ${isGroupActiveNow ? "bg-white/10 border-white/20" : "bg-slate-50 dark:bg-slate-800/50 border-transparent"}`}
                >
                  {group.sessions.map((sess) => {
                    const isSessActive =
                      group.day === todayName &&
                      curT >= sess.startTime.substring(0, 5) &&
                      curT <= sess.endTime.substring(0, 5);
                    return (
                      <div
                        key={sess.scheduleId}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span
                            className={`text-[10px] font-bold tracking-tight ${isGroupActiveNow ? "text-white/90" : "text-slate-600 dark:text-slate-400"}`}
                          >
                            {sess.startTime.substring(0, 5)} —{" "}
                            {sess.endTime.substring(0, 5)}
                          </span>
                        </div>
                        {isSessActive && (
