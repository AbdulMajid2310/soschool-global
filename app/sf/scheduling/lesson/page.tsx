...
import {
  IGroupedSchedule,
  ISession,
} from "@/redux/features/school_schedule/types";
    ...
    <div className="p-6 md:p-8 min-h-screen ">
      <div className="max-w-6xl mx-auto flex justify-between items-center mb-6">
        <h1 className="text-4xl font-black italic uppercase tracking-tighter">
          Jadwal<span className="text-blue-600">.</span>
        </h1>
        <button
          onClick={() => router.push("lesson/add")}
          className="bg-blue-600 text-white px-6 py-3 rounded-xl font-black text-xs tracking-widest hover:scale-105 transition-all shadow-lg shadow-blue-600/20"
        >
          TAMBAH DATA
        </button>
      </div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
            {days.map((day) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-xl text-[10px] font-black transition-all border-2 shrink-0 ${
                  activeDay === day
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {timeSlots.map((time) => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-[9px] font-black transition-all shrink-0 uppercase tracking-tighter ${
                  activeTime === time
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredData.length > 0 ? (
          filteredData.map((item, idx) => {
            ...
            const hasActiveSession = item.sessions.some(
              (s) => curT >= s.startTime && curT <= s.endTime,
            );

            return (
              <div
                key={idx}
                className={`relative bg-white dark:bg-slate-900 p-5 rounded-3xl border-2 transition-all duration-500 group ${
                  hasActiveSession
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <span
                    className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${hasActiveSession ? "bg-blue-600 text-white animate-pulse" : "bg-slate-100 dark:bg-slate-800 text-slate-500"}`}
                  >
                    {item.classroom.classroom.name || "class"}
                  </span>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {item.sessions.map((sess) => (
                      <div
                        key={sess.scheduleId}
                        className="flex items-center justify-between group/sess bg-slate-50 dark:bg-slate-800/40 p-2.5 rounded-xl border border-transparent hover:border-blue-200 transition-all"
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-2 h-2 rounded-full ${curT >= sess.startTime && curT <= sess.endTime ? "bg-blue-600 animate-ping" : "bg-slate-300"}`}
                          />
                        ...
                        <div className="flex gap-1 opacity-0 group-hover/sess:opacity-100 transition-opacity">
                          ...
                          <button
                            title="hapus"
                            onClick={() =>
                              handleDelete(sess, item.subject.name)
                            }
