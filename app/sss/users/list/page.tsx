...
export default function UserListSection() {
    ...
    const filteredUsers = useMemo(() => {
        return users.filter((user) =>
            ...
            (user.registrationNumber && user.registrationNumber.toLowerCase().includes(searchQuery))
        );
    }, [users, searchQuery]);
    ...
    if (loading && users.length === 0) {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse p-4">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div key={i} className="h-64 bg-slate-100 dark:bg-slate-800 rounded-4xl w-full" />
                ))}
            </div>
        );
    }
        ...
        <div className="space-y-8 animate-in p-4 fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => router.back()}
                        className="p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all group"
                    >
                        <HiOutlineArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
                    </button>
                    <div>
                        <h2 className="text-3xl font-black italic uppercase tracking-tighter dark:text-white leading-none">List Pengguna</h2>
                        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] italic mt-1">Management Database User</p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    <div className="relative flex-1 md:w-80 group">
                        <HiOutlineMagnifyingGlass className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors" size={18} />
                        <input
                            type="text"
                            value={searchTerm}
                ...
                {filteredUsers.map((user) => (
                    ...
                    >
                        ...
                        <div className="absolute top-4 right-4">
                            {user.isVerified ? (
                                <div className="text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 p-1.5 rounded-full" title="Verified">
                                    <HiOutlineCheckBadge size={18} />
                                </div>
                            ) : (
                                <div className="text-amber-500 bg-amber-50 dark:bg-amber-500/10 p-1.5 rounded-full" title="Pending">
                        ...
                        <div className="mt-6 grid grid-cols-3 gap-3">
                            ...
                            <button className="flex items-center justify-center gap-2 py-2.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 rounded-2xl hover:bg-indigo-600 hover:text-white transition-all text-[10px] font-black uppercase italic">
