      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium italic uppercase tracking-widest">
            Internal Operations & Team Governance
          </p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => router.push("/sss/internal-users/config")}
            className="bg-white/5 border border-white/10 px-6 py-3 rounded-2xl text-[10px] font-black uppercase italic hover:bg-white/10 transition-all flex items-center gap-2"
          >
            <FiSettings /> System Config
          </button>
          <button
            onClick={() => router.push("/sss/internal-users/staff")}
            className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase italic transition-all shadow-lg shadow-blue-600/20 flex items-center gap-2"
          >
          ...
          <div className="bg-white/5 border border-white/10 rounded-4xl overflow-hidden backdrop-blur-xl">
            <div className="p-8 border-b border-white/5 bg-white/2 flex justify-between items-center">
              <h3 className="text-xs font-black uppercase italic text-white flex items-center gap-2 tracking-widest">
                <FiLifeBuoy className="text-blue-500" /> Keluhan Admin Sekolah
                (Active Tickets)
              </h3>
              <div className="flex gap-2">
                <button
                  type="button"
                  title="pencarian"
                  className="p-2 bg-white/5 rounded-lg text-slate-500"
                >
                  <FiSearch />
                </button>
                <button
            ...
            <div className="divide-y divide-white/5">
              {SUPPORT_TICKETS.map((ticket) => (
                <div
                  key={ticket.id}
                  className="p-8 hover:bg-white/1 transition-all group flex flex-col lg:flex-row justify-between gap-6"
                >
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-2 h-2 rounded-full ${ticket.priority === "Urgent" ? "bg-rose-500 animate-pulse" : "bg-amber-500"}`}
                      />
                      <h4 className="text-sm font-black text-white uppercase italic tracking-tight">
                        {ticket.issue}
                      </h4>
                      <span className="text-[8px] font-black bg-white/5 px-2 py-0.5 rounded text-slate-500 uppercase">
                        {ticket.id}
                      </span>
                    </div>
                    <div className="flex flex-wrap gap-4 text-[9px] font-black text-slate-500 uppercase italic">
                      <span className="flex items-center gap-1">
                        <FiUsers className="text-blue-500" /> {ticket.school}
                      </span>
                      <span className="flex items-center gap-1">
                        <FiCpu className="text-purple-500" /> Category:{" "}
                        {ticket.category}
                      </span>
                      <span className="flex items-center gap-1 text-slate-600">
                        <FiClock /> {ticket.timestamp}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right mr-4">
                      <span
                        className={`text-[9px] font-black uppercase italic px-3 py-1 rounded-full border ${
                          ticket.status === "Open"
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-4xl p-8 backdrop-blur-xl">
              ...
              {STAFF_MEMBERS.map((staff, idx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-[11px] font-black text-white uppercase italic leading-none mb-1">
                        {staff.name}
                      </h4>
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${staff.status === "Online" ? "bg-emerald-500" : "bg-slate-700"}`}
                    />
            ...
            <div className="mt-8 pt-8 border-t border-white/5">
              <p className="text-[10px] font-black text-slate-500 uppercase italic mb-4 tracking-widest">
                Recent Access Logs
              </p>
...
const HealthBar = ({ label, status, value }: any) => (
  <div className="space-y-1.5">
    <div className="flex justify-between text-[9px] font-black uppercase italic">
      ...
      <span className={value > 90 ? "text-emerald-500" : "text-amber-500"}>
        {status}
      </span>
    </div>
    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
      <div
        className={`h-full ${value > 90 ? "bg-emerald-500" : "bg-amber-500"} w-[${value}%]`}
      />
    </div>
  </div>
);
