...
import { FiBookOpen, FiMoreVertical, FiPlus, FiUsers, FiClock, FiLayers } from 'react-icons/fi';
...
export const TEACHER_SUBJECTS: Subject[] = [
  { id: '1', name: 'Informatika', class: '10 - IPA 1', studentsCount: 36, progress: 75, color: 'bg-indigo-500', nextSync: 'Selasa, 08:00' },
  { id: '2', name: 'Algoritma & Struktur Data', class: '11 - RPL 2', studentsCount: 32, progress: 40, color: 'bg-violet-500', nextSync: 'Rabu, 10:30' },
  { id: '3', name: 'Basis Data', class: '12 - RPL 1', studentsCount: 30, progress: 90, color: 'bg-fuchsia-500', nextSync: 'Kamis, 13:00' },
];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] italic mb-1">Akademik</h2>
          <p className="text-xl font-black italic uppercase text-slate-900 dark:text-white">Jadwal & Materi Ajar</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {TEACHER_SUBJECTS.map((subject) => (
          <div key={subject.id} className="group relative bg-white dark:bg-[#0a0f1d] rounded-[2.5rem] p-6 border border-slate-200 dark:border-indigo-900/20 shadow-sm hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500">
            
            {/* Top Info */}
            <div className="flex justify-between items-start mb-6">
              <div className={`w-14 h-14 ${subject.color} rounded-3xl flex items-center justify-center text-white text-2xl shadow-lg group-hover:rotate-6 transition-transform`}>
                <FiBookOpen />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
                ...
                <span className="text-[10px] font-bold uppercase tracking-tighter">{subject.studentsCount} Siswa</span>
              </div>
              <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
                <FiClock className="text-indigo-500" />
                <span className="text-[10px] font-bold uppercase tracking-tighter">{subject.nextSync}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-[9px] font-black uppercase text-slate-400 italic">Kurikulum</span>
                <span className="text-[9px] font-black text-indigo-500">{subject.progress}%</span>
              </div>
        ...
        <button className="border-2 border-dashed border-slate-200 dark:border-indigo-900/20 rounded-[2.5rem] p-6 flex flex-col items-center justify-center gap-3 text-slate-400 hover:border-indigo-500 hover:text-indigo-500 transition-all group">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest italic">Tambah Mapel Baru</span>
        </button>
      </div>
    </div>
  );
