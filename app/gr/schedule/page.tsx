...
export default function AgendaCard() {
  ...
  const { currentTeacherProfile, selectedTeacherId } = useAppSelector(
    (state) => state.teacher,
  );
  const teacherId = currentTeacherProfile?.teacherId;
  const { summary, schedules } = useAppSelector(
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-6">
        <StatsSchedule
          ...
          value={summary?.totalTeachingHours || "0 Jam"}
          sub="Minggu ini"
          color="text-amber-500"
        />
        <StatsSchedule
          icon={<FiCheckCircle />}
          label="Materi Terselesaikan"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-linear-to-br from-amber-500 to-orange-600 p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden group">
            <FiTrendingUp
              className="absolute -right-6 -top-6 text-white/10 group-hover:scale-110 transition-transform duration-700"
              size={150}
            />
            <h4 className="text-[10px] font-black uppercase tracking-widest text-amber-100 mb-6 italic">
              Pacing Analysis (AI)
            </h4>
            <div className="space-y-6 relative z-10">
              <p className="text-xs font-medium leading-relaxed italic">
                "Berdasarkan jadwal, Anda memiliki **3 jam jeda** hari ini.
                Waktu terbaik untuk mengoreksi tugas kelas **11-RPL-2** adalah
                pukul 15:00 saat energi fokus Anda sedang stabil."
              </p>
              <div className="p-4 bg-white/10 backdrop-blur-md rounded-2xl border border-white/10">
                <p className="text-[9px] font-black uppercase tracking-widest mb-2 text-amber-200">
                  Rekomendasi Istirahat
                </p>
                <p className="text-[10px] font-bold italic">
                  10:00 - 10:30 (Recharge Time)
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#0a0f1d] p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/5">
            <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 italic">
              Next Week Preview
            </h5>
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-[10px] font-bold italic">
                ...
                <span className="text-slate-400 text-glow-indigo">
                  Materi Baru: API Dev
                </span>
              </div>
            </div>
