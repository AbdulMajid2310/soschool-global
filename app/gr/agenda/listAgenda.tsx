      ...
      <div className="space-y-4 relative before:absolute before:left-8 before:top-0 before:bottom-0 before:w-px before:bg-slate-100 dark:before:bg-white/5">
        {schedules.map((item, index) => (
          <div key={index} className="relative pl-16 group">
            {/* Timeline Dot */}
            <div
              className={`absolute left-7.5 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full border-4 border-white dark:border-[#0a0f1d] z-10 transition-all duration-500 group-hover:scale-150
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <button
                    type="button"
                    title="next"
                    onClick={() => handleDetail(item.subject.subjectId)}
                    className="p-4 bg-slate-50 dark:bg-white/5 rounded-2xl text-slate-400 group-hover:bg-amber-500 group-hover:text-white transition-all"
                  >
                    <FiArrowRight />
                  </button>
                </div>
